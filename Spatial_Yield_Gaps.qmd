...
tau_hat$mu_c<--(model$model$`log(L.tonPerHectare)`-tau_hat$mu)/tau_hat$sigma_v^2*tau_hat$sigma_c^2#object$family$s*mgcv::residuals.gam(object)/sigma_v^2*sigma_c^2
...
effdt_battese=efficiency(model,type="battese")
lon=model$model$O.largestPlotGPS.Longitude
lon=as.data.frame(lon)
lat=model$model$O.largestPlotGPS.Latitude
lat=as.data.frame(lat)
effdt_battese=cbind(lon,lat,effdt_battese)
...
# Efficiency kriging
# pred_eff <- expand.grid(O.largestPlotGPS.Longitude = seq(82, 89, length = 100),O.largestPlotGPS.Latitude = seq(24,28, length = 100))
# 
# pred_eff$Sowing_Date_Early=1
# pred_eff$Nperha=mean(model$model$Nperha)
# pred_eff$wc2.1_30s_elev=mean(model$model$wc2.1_30s_elev)
# pred_eff$Weedmanaged=1
# pred_eff$temp=mean(model$model$temp)
# pred_eff$precip=mean(model$model$precip)
# 
# tau_hat_eff <- predict(model,newdata=pred_eff)
# 
# tau_hat_eff=as.data.frame(tau_hat_eff)
# 
# names(tau_hat_eff)[1:3]=c("mu","sigma_v","sigma_u")
# 
# 
# tau_hat_eff$sigma_c<-sqrt((tau_hat_eff$sigma_u^2*tau_hat_eff$sigma_v^2)/(tau_hat_eff$sigma_v^2+tau_hat_eff$sigma_u^2))#(1/sigma_u^2+1/sigma_v^2)^(-1)
# 
# 
# 
# tau_hat_eff$mu_c<--(model$model$`log(L.tonPerHectare)`-tau_hat_eff$mu)/tau_hat_eff$sigma_v^2*tau_hat_eff$sigma_c^2#object$family$s*mgcv::residuals.gam(object)/sigma_v^2*sigma_c^2
# 
# 
# 
# tau_hat_eff$u<-exp(1/2*tau_hat_eff$sigma_c^2-tau_hat_eff$mu_c)*stats::pnorm(tau_hat_eff$mu_c/tau_hat_eff$sigma_c-tau_hat_eff$sigma_c)/stats::pnorm(tau_hat_eff$mu_c/tau_hat_eff$sigma_c)
# 
# 
# pred_tau_hat=cbind(pred_eff,tau_hat_eff)
